...
def cpf_analize(cpf_number):    
    ...
    response = requests.get(api_serpro_cpf_url + str(cpf_number), headers = headers) 
    json_object = json.loads(json.dumps(response.json()))
    return json.dumps(json_object, indent=2)


def person_insurance_simulation(cnpj, codigoModeloProposta, name, cpf, birthDate, work, earn, gender, uf ):
    ...
    response = requests.post(api_mongeral_aegon_insurance, params = params, headers = headers, json= body )
    json_object = json.loads(json.dumps(response.json()))
    return json.dumps(json_object, indent=2)
